#!/usr/bin/env bash
# Terminal diagnostics: capture environment and CLI settings to debug recurring terminal issues.
# Safe defaults to avoid interactivity or pagers.
set -euo pipefail
...
echo
echo "Tip: To hard-disable prompts/pagers for sessions, you can export:"
echo "  export GIT_PAGER=cat GH_PAGER=cat PAGER=cat LESS=FRSX"
echo "  gh config set prompt disabled"
echo "  git config --global core.pager cat"
